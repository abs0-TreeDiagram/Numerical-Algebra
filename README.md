...
与本库函数兼容的矩阵必须通过库函数创建，n维向量视作n行1列的矩阵（列向量）。`matCreate`用于新建矩阵，`matCopy`用于复制矩阵，`matTransfer`用于将C数组转化为兼容矩阵。这些函数都将返回一个代表所创建矩阵的二级地址。

`example.c`提供了一个幂法求谱半径的示例。

库函数封装工作仍在进行中，本文件夹会不定时更新。可以打开`numalg.h`文件查看当前可用的函数。

### 更新日志

2026年4月2日 添加幂法的Atiken加速；添加矩阵的QR分解及配套基础函数；修复若干问题. 

2026年4月4日 添加基本QR法求矩阵特征值；修复了QR分解的问题。

2026年4月7日 添加Householder标准变换、矩阵拟上三角化函数、简单LU分解及解方程组、反幂法求特征值。
