...
double * mat_mult(double * A, int i, int j, double * B, int k, int l)
{
    if (j != k)
    {
        printf("ERROR: Invalid matrix multiplication action, with first column size: %d and second row size: %d.\n", j, k);
        exit(-1);
    }
    else
    {
        // Perform matrix multiplication
        double * C = (double *)malloc(i * l * sizeof(double));
        for (int m = 0; m < i; m++)
        {
            for (int n = 0; n < l; n++)
            {
